...
variable "vsphere_port_group_eth3" {
  description = "In which port group the VM NIC will be configured for eth3 interface (default: VM Network)"
}
...
#variable "ip_type" {
#  description = "VM-Series IP address (Static or DHCP Client)"
#}
...
variable "auth-key" {
  description = "VM-Series license auth-key (Optional)"
}